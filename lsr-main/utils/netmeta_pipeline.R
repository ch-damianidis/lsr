...
build_nm <- function(data, random = TRUE, reference.group = NULL) {
...
run_cnma_analysis <- function(data, interaction = FALSE, random = TRUE,
  ...
  if (connected) {
    ...
    if (interaction) {
      # Extract all unique components from treatment names
      all_trts <- unique(c(data$treat1, data$treat2))
      comps <- sort(unique(trimws(unlist(strsplit(all_trts, "\\s*\\+\\s*")))))

      # Generate all 2-way interaction terms in the correct format ("X+Y")
