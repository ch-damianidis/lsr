...
ui <- fluidPage(
  ...
  titlePanel("Living NMA Tool (CLL â€“ Demo)"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_nma, {
    ...
    if (all(required_cols_arm %in% colnames(data))) {
      ...
      output$data_validation_msg <- renderText("Pairwise data detected.")   # Message in Data Overview if data are already pairwise
    }
  ...
  # ---- Treatment-level NMA (used for inconsistency tools) ----
  nm_model <- reactive({
    req(raw_data())
    tryCatch({
  ...
  output$network_plot <- renderPlot({
    ...
    tryCatch({
      netmeta::netgraph(
        nm,
        number.of.studies = TRUE,                # show number of studies on each edgå
        thickness         = "number.of.studies", # edge width by number of studies
        multiarm          = TRUE,                # proper multi-arm display
        points            = TRUE,                # draw nodes
        cex.points        = 1.4,                 #  node size
        cex               = 1.1,                 # label size
        plastic           = FALSE,               #  clean layout
        seq               = sort(nm$trts)        #stable treatment order for nicer layout
        # optional
        # col              = "black",
        # col.points       = "black",
        # col.multiarm     = "grey40"
  ...
  observeEvent(input$confirm_merge, {
    ...
    # Replace selected components with the new family name within each treatment label
    ...
    if(nrow(self_loops) > 0){
      ...
      raw_data(new_data)            # <---- # updating the reactive data will retrigger dependent reactives/outputs
  ...
  output$league_table <- renderTable({
    nm  <- nm_model()                      # treatment-level netmeta object
    dat <- raw_data()                      # current data (for connectivity check)
    validate(need(!is.null(nm),  "Model is not available."))
    validate(need(!is.null(dat), "Data not available."))
    
    # connectivity check via igraph on the actual edges
    edges <- data.frame(from = dat$treat1, to = dat$treat2, stringsAsFactors = FALSE)
    g <- igraph::graph_from_data_frame(edges, directed = FALSE)
    is_conn <- igraph::is.connected(g)
    
    if (!is_conn || length(nm$trts) < 3) {
      return(data.frame(Message = "League table not applicable to this network"))
    }
    
    tryCatch({
      lt <- netmeta::netleague(nm, bracket = "both", digits = 3)
      as.data.frame.matrix(lt)
    }, error = function(e) {
      data.frame(Message = paste("Failed to generate league table:", conditionMessage(e)))
    })
  }, rownames = TRUE)
  ...
  output$ranking_table <- renderTable({
    ...
    tryCatch({
      ps <- netmeta::netrank(nm, small.values = "good")  # HR: smaller is better
      data.frame(
      ...
      data.frame(Message = paste("Failed to calculate treatment ranking:", conditionMessage(e)))
    })
  }, rownames = FALSE)
  ...
  output$netsplit_table <- renderTable({
    ...
    out <- res[, c("comparison","Direct","Indirect",
  ...
  output$nma_summary <- renderPrint({
    ...
    #  Fallback message if interaction requested but network is disconnected
